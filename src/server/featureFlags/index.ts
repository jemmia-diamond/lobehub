 ...
 */
export const getServerFeatureFlagsFromEdgeConfig = async (userId?: string, userEmail?: string) => {
  ...
  if (EdgeConfig.isEnabled()) {
    try {
      ...
      if (edgeFeatureFlags && Object.keys(edgeFeatureFlags).length > 0) {
        ...
        debug('Using EdgeConfig flags for user: %s (%s)', userId || 'anonymous', userEmail || 'no-email');
        return mergedFlags;
      } else {
  ...
  debug('Using environment variable flags for user: %s (%s)', userId || 'anonymous', userEmail || 'no-email');
  return envFlags;
};
...
export const getServerFeatureFlagsStateFromEdgeConfig = async (
  ...
  const flags = await getServerFeatureFlagsFromEdgeConfig(userId, userEmail);
