...
describe('getCanonicalUrl', () => {
  ...
  it('should return correct URL for production environment', async () => {
    ...
    expect(result).toBe('https://lobehub.jemmia.vn/path/to/page');
    expect(urlJoin).toHaveBeenCalledWith('https://lobehub.jemmia.vn', 'path', 'to', 'page');
  });
  ...
  it('should work correctly without additional path arguments', async () => {
    ...
    expect(result).toBe('https://lobehub.jemmia.vn');
    expect(urlJoin).toHaveBeenCalledWith('https://lobehub.jemmia.vn');
  });
});
