...
| Lark                  | `lark`                  | `AUTH_LARK_APP_ID`, `AUTH_LARK_APP_SECRET`                                                                         |
...
<Cards>
  ...
  <Card href={'/docs/self-hosting/auth/providers/lark'} title={'Lark'} />
...
- **Built-in Providers**: `https://yourdomain.com/api/auth/callback/{provider}`
- **Generic Providers**: `https://yourdomain.com/api/auth/oauth2/callback/{provider}`

*(Substitute `http://localhost:3210` for development)*
...
1. Create a "Custom App" at [Lark Open Platform](https://open.larksuite.com/)
2. Enable "Web App" and set redirect URL: `https://yourdomain.com/api/auth/oauth2/callback/lark`
3. Scope requirements: `contact:user.base:readonly`, `contact:user.email:readonly`
4. Configure environment variables:

```bash
AUTH_SSO_PROVIDERS=lark
AUTH_LARK_APP_ID=cli_xxxxxxxxxxxxxx
AUTH_LARK_APP_SECRET=xxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
